...
실전 면접에서 느끼는 긴장감이나 말하기 연습은 텍스트 기반 AI 챗봇만으로는 대체하기 어려웠습니다. 이러한 문제의식을 갖고 있던 중, 마침 비슷한 아이디어를 구현한 강의를 발견했습니다. 강의의 초기 뼈대를 출발점으로 삼되, **실서비스 환경에서 실제로 쓸 수 있는 수준으로 만드는 것을 목표로** 직접 설계하고 고도화했습니다.
...
- **측정 기반 성능 최적화:** 폰트 최적화와 SSR prefetching 적용으로 LCP를 13.6초에서 5.4초로, FCP를 2.5초에서 0.9초로 크게 단축했습니다. 동적 import를 통해 번들 크기를 30kb 줄여 초기 로딩 성능을 개선했습니다.
...
이번 프로젝트를 통해 단순 기능 구현을 넘어, 렌더링 전략과 컴포넌트 구조, 성능 최적화를 실제 지표로 검증하는 경험을 쌓았습니다.

- **배운 점**:

  - 서버 컴포넌트와 클라이언트 컴포넌트의 경계를 재설계하고 SSR prefetching을 적용하여 FCP를 2.5s → 0.9s로 크게 개선
  - UI와 비즈니스 로직의 관심사 분리를 통해 단위 테스트 작성의 복잡도를 낮추고, 변경 영향 범위를 축소
  - 실시간 음성 인터랙션에서 지연 시간과 체감 속도를 조정하며 도메인에 맞는 UX 응답 기준을 수립

- **개선할 점**:

  - 테스트 코드 작성 시 `data-testid`에 대한 의존도가 높았던 이유는 시맨틱 구조 부족이었으며, 앞으로는 마크업 단계에서부터 접근성을 고려한 구조 설계가 필요하다는 점을 확인
  - 동적 임포트로 번들 크기를 약 30kb 줄였지만, 초기 설계 단계에서 번들 분석 및 코드 스플리팅 전략을 함께 고려하지 못한 점은 아쉬움
